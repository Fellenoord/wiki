...
For the scripts found in the `package.json` file, you can run them by typing `pnpm <script-name>`. For example, if you want to run the backend in such a way that it restarts on every file change, you would run `pnpm start:dev`. If you want to run the frontend in development mode, you would run `pnpm dev`.
...
- `test:e2e`: Runs all e2e tests, can supply a pattern to run specific tests
   ...
   - `-t 'PATTERN'`: Runs all e2e test cases that contain the pattern `PATTERN`.
        > A use case example might be the following. Say you want to run a test called 'should fail if the user does not have the required roles' in the `news.e2e-spec.ts` file. You would run `pnpm test:e2e news -t 'should fail if the user does not have the required roles'`.
...
For the database scripts, you'll need to run `pnpm db:migration` if you have made any changes to the Prisma schema file and want these to be reflected in the database. This will also automatically regenerate the Prisma types. You might need to run `pnpm db:gen` if, for whatever reason, you find that your Prisma types do not match the schema. If you do not have the seeded data (such as some default users) in your database, you can run `pnpm db:seed` to populate the database with this data. If something went horribly wrong with your local database, or you just want to sync your database with the latest schema, you can run `pnpm db:sync` which will entirely drop your local database and recreate it from scratch, note that any data you have in your local database will be lost.
...
If you are creating additional GraphQL queries, mutations or fragments or if the schema has changed on the backend, you will need to run `pnpm gen` to regenerate the GraphQL types and queries. This will also tell you if there are any errors in your queries, mutations or fragments.
